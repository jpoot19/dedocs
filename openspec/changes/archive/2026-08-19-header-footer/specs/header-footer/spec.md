# Delta for header-footer

## Purpose

Add header and footer band support to dedocs via a slots-only API. Bands are reserved space on every page that repeats content but does not participate in body pagination. New `header-footer` domain (no prior spec).
